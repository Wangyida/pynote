...
def VAEGAN(input_shape=[None, 784],
    ...
    return {'x_img': x_img, 'x_obj': x_obj, 'z': z, 'x_tilde': decoding_actual['x_tilde'],
...
def train_vaegan(files_img,
    ...
    test_xs_img = sess.run(batch_img) / 255.0
    montage(test_xs_img, 'test_xs_img.png')
    test_xs_obj = sess.run(batch_obj) / 255.0
    montage(test_xs_obj, 'test_xs_obj.png')
    try:
...
if __name__ == '__main__':
    train_vaegan(files_img=files_img, files_obj=files_obj)

...
def GAN(input_shape, n_latent, n_features, rgb, debug=True):
    ...
    x_img = (x_img / 127.5) - 1.0
    sum_x_img = tf.image_summary("x_img", x_img)
    x_obj = tf.placeholder(tf.float32, input_shape, 'x_obj')
    x_obj = (x_obj / 127.5) - 1.0
    sum_x_obj = tf.image_summary("x_obj", x_obj)
    phase_train = tf.placeholder(tf.bool, name='phase_train')

    # Discriminator for real input samples
    D_real_logits = discriminator(
        x_img, phase_train, n_features=n_features, rgb=rgb)
...
def train_ds():
    ...
    try:
        while not coord.should_stop():
            batch_xs_img = sess.run(batch_img)
            batch_xs_obj = sess.run(batch_obj)
            step_i += 1
